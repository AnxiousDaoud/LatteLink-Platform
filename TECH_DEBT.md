...
Resolution: Change default to `staff`. Enforce at onboarding. See ROADMAP Phase 0.
...
Resolution: Add Sentry, structure logs as JSON, set up basic alerting. See ROADMAP Phase 2.
...
Resolution: Managed container platform (Fly.io/Railway). See ROADMAP Phase 6.
...
**TD-10** — Hardcoded `rawaqcoffee` defaults in catalog/tenant.ts  
...
**TD-11** — Mobile app is single-tenant per EAS build  
...
Resolution: Accept this as the operational model (separate branded apps) with documented per-merchant build process, or invest in dynamic merchant routing. Decision required in Phase 1.

**TD-12** — Fallback menu/config data in mobile catalog.ts  
...
**TD-13** — No push notification receipt polling  
...
Resolution: Add a Expo push receipt polling worker before Phase 3 (campaigns), when delivery failures become commercially significant.

**TD-14** — Loyalty is fixed 1pt = 1¢, not merchant-configurable  
...
Resolution: Create `loyalty_programs` table per merchant. See ROADMAP Phase 3.

**TD-15** — Passkey UI missing on mobile (server is ready)  
...
Resolution: Add passkey register/auth flow to the mobile auth screen. See ROADMAP Phase 2.

**TD-16** — Admin console owner page has copy for reset flows not implemented  
...
**TD-17** — `docs/architecture/architecture-overview.md` is stale and misleading  
...
Resolution: Rewrite. See ROADMAP Phase 0.

**TD-18** — `CHANGELOG.md` is empty  
...
**TD-19** — `repo-files.md` appears to be a stale generated file list  
...
**TD-20** — `packages/persistence/src/index.ts` contains deprecated table-bootstrap code  
...
**TD-21** — `docs/implementation/final-25-percent-*.md` are ephemeral development artifacts  
...
**TD-22** — Duplicate migration numbering (`0015_home_news_cards` and `0015_identity_customer_birthday`)  
